...
def create_health(entry: schemas.HealthEntryCreate, db: Session = Depends(get_db_session)):
...
def list_health(
    ...
    query = list_entries(query, models.HealthEntry, start_date, end_date, limit)
    return query.all()
...
def delete_health(entry_id: int, db: Session = Depends(get_db_session)):
    record = db.get(models.HealthEntry, entry_id)
    if not record:
        raise HTTPException(status_code=404, detail="Health entry not found")
    db.delete(record)
